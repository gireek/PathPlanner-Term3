        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            for(int i = 0; i<sensor_fusion.size(); i++){
              float d = sensor_fusion[i][6]; // the 6th value of ith car on road which helps in knowing lane
              if(d<(2+4*lane+2) && d>(2+4*lane-2)){
                  double vx = sensor_fusion[i][3]; //speed of car in x direction
                  double vy = sensor_fusion[i][4];
                  double check_speed = sqrt(vx*vx+vy*vy); // magnitude of speed
                  double check_car_s = sensor_fusion[i][5]; //s value of car
                  check_car_s+= ((double)prev_size*0.02*check_speed); //helps to predict car location in future
                  if(check_car_s>car_s && check_car_s-car_s<30)
                  {
                    //if within 30 m of this car in future
                    too_close = true;
                    int l = lane + 1;
                    int right_try = 0;
                    if(right_try==0){
                      if (l < 3 && l>=0){
                        ...
                        for(int i = 0; i<sensor_fusion.size(); i++){
                          ...
                          if(d<(2+4*l+2) && d>(2+4*l-2)){
                            ...
                            if(abs(check_car_s-car_s) < 20){safe = 0;}
                          }
                        }
                        if (safe==1){lane = l;right_try=1;}
                      }
                    }
                    //if right lane change was unsuccessful
                    if(right_try==0){
                      l = l-2;
                      if (l < 3 && l>=0){
                        int safe = 1;
            ...
            //SPLINE
            //list of widely spaced (x,y) waypoints spaced at 30 m 
             vector <double> ptsx;
             vector <double> ptsy;
             //reference point is where the car is at present and at what angle
             double ref_x = car_x;
             double ref_y = car_y;
             double ref_yaw = deg2rad(car_yaw);
             //the previous path size is either going to be close to empty
             if (prev_size < 2){
            ...
            //ptsx and ptsy have been created with 5 points
            //shift and rotation to set car as origin in reference frame and angle also at 0 degrees
            ...
            tk::spline s;
            s.set_points(ptsx,ptsy); // make the spline of these points
